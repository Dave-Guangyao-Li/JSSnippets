...
function palindrome(str) {
    // format the string
    const regex = /[A-Za-z0-9]/g; // match only alphanumeric
    //check for non-alphanumeric chars and remove and convert case
    const strArr = [...str.matchAll(regex)].map(match=>match[0]).map(char=>char.toLowerCase());
    console.log(strArr);
